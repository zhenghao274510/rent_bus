<template>
  <div class="choseshop">
    <div class="chosehead">
        <a @click="back"><i class="icon back"></i></a>
         <span>选择门店</span>
        <router-link to=""><i class="icon search"></i></router-link>
    </div>
   <div class="chosecont" ref="all">
    <!-- 左侧导航 -->
  <div class="left" ref="left">
    <ul >
           <li v-for="(item,index) in list" :key="index" ref='p' @click=" selectItem(index,$event)" :class="{'active':currentIndex===index}">{{ item }}</li>
        </ul>
  </div>
 <!-- 右侧内容 -->
